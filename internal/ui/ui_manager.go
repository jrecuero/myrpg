...
// DrawTopPanel renders the player information panel
func (ui *UIManager) DrawTopPanel(screen *ebiten.Image, activePlayer *components.RPGStatsComponent) {
	// Draw background
	vector.FillRect(screen, 0, 0, ScreenWidth, TopPanelHeight, TopPanelColor, false)
	
	if activePlayer == nil {
		ebitenutil.DebugPrintAt(screen, "No active player", 10, 10)
		return
	}
	
	// Player basic info (left side)
	playerInfo := fmt.Sprintf("Active: %s (%s Level %d)", 
		activePlayer.Name, activePlayer.Job.String(), activePlayer.Level)
	ebitenutil.DebugPrintAt(screen, playerInfo, 10, 10)
	
	// Health and Mana (left side, second line)
	hpMpInfo := fmt.Sprintf("HP: %d/%d  MP: %d/%d", 
		activePlayer.CurrentHP, activePlayer.MaxHP, 
		activePlayer.CurrentMP, activePlayer.MaxMP)
	ebitenutil.DebugPrintAt(screen, hpMpInfo, 10, 30)
	...
	ebitenutil.DebugPrintAt(screen, combatInfo, 400, 10)
	
	// Experience (right side, second line)
	expInfo := fmt.Sprintf("EXP: %d/%d", 
		activePlayer.Experience, activePlayer.ExpToNext)
	ebitenutil.DebugPrintAt(screen, expInfo, 400, 30)
	
	// HP Bar (visual representation)
	ui.drawHealthBar(screen, 10, 50, 200, 10, activePlayer.CurrentHP, activePlayer.MaxHP)
	
	// MP Bar (visual representation)  
	ui.drawManaBar(screen, 220, 50, 150, 10, activePlayer.CurrentMP, activePlayer.MaxMP)
}
...
func (ui *UIManager) DrawGameWorldBackground(screen *ebiten.Image) {
	...
	vector.FillRect(screen, 0, GameWorldY, ScreenWidth, GameWorldHeight, gameWorldColor, false)
}
